...
//#region office → PDF visual conversion (MS Office COM, cached)
/** Absolute cache dir for converted PDFs (under DSH_HOME/storages). */
function pdfCacheDir() {
	return join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "storages", "file-preview-cache");
}

/** Plugin directory (where convert-office.ps1 lives beside this module). */
const pluginDir = dirname(fileURLToPath(import.meta.url));

/** COM converter kind for an Office format, or null when unsupported. */
export function officeConverterKind(format) {
	if (format === "docx" || format === "doc") return "word";
	if (format === "xlsx" || format === "xls") return "excel";
	if (format === "pptx" || format === "ppt") return "powerpoint";
	return null;
}

function pdfCacheKey(path, size, mtimeMs) {
	return createHash("sha256").update(`${path}\u0000${size}\u0000${mtimeMs}`).digest("hex");
}

/** Windows PowerShell 5.1 (best COM support); full path avoids PATH gaps. */
const POWERSHELL = process.platform === "win32"
	? "C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe"
	: "powershell";

/** In-flight conversions: cache key → started-at (ms), to avoid duplicate spawns. */
const inFlight = new Map();

/** Spawn the converter detached (fire-and-forget) writing straight to the cache PDF. */
function spawnConverter(kind, srcPath, outPath) {
	const script = join(pluginDir, "convert-office.ps1");
	try {
		const child = spawn(POWERSHELL, [
			"-NoProfile", "-ExecutionPolicy", "Bypass", "-File", script,
			"-Kind", kind, "-Src", srcPath, "-Out", outPath
...
/**
 * Return a cached PDF for visual preview, or kick off a background conversion.
 * @returns absolute PDF path when ready; null when converting (or unsupported).
 */
export async function convertOfficeToPdf(path, format, size, mtimeMs) {
	const kind = officeConverterKind(format);
	if (kind === null) return null;
	const pdfPath = join(pdfCacheDir(), `${pdfCacheKey(path, size, mtimeMs)}.pdf`);
	try {
		await stat(pdfPath);
		inFlight.delete(pdfCacheKey(path, size, mtimeMs));
		return pdfPath; // cached
	} catch {
		/* convert below */
	}
	const key = pdfCacheKey(path, size, mtimeMs);
	const now = Date.now();
	for (const [k, started] of inFlight) {
		if (now - started > 600000) inFlight.delete(k); // stale guard
	}
	if (inFlight.has(key)) return null; // already converting
	await mkdir(pdfCacheDir(), { recursive: true });
	inFlight.set(key, now);
	spawnConverter(kind, path, pdfPath);
	return null;
}
//#endregion
...
async function handlePreview(ctx, req, res) {
	...
	try {
		...
		if (kind === "office") {
			...
			// 1) Visual preview: convert to PDF (MS Office COM), cached.
			try {
				const pdf = await convertOfficeToPdf(path, format, info.size, info.mtimeMs);
				if (pdf !== null) {
					json(res, 200, { ...base, kind: "pdf", officeFormat: format, url: `${RAW_PATH}?path=${encodeURIComponent(pdf)}` });
					return;
				}
			} catch (error) {
				ctx.logger.warn(`file-preview: office→pdf conversion failed for "${path}": ${String(error)}`);
			}
			// Conversion running in the background: tell the client to poll.
			if (officeConverterKind(format) !== null) {
				json(res, 200, { ...base, kind: "pending", officeFormat: format });
				return;
			}
			// 2) Fallback: extracted text (no converter available).
			try {
...
/** Full paths for native launch commands: the server's PATH may be stripped. */
...
function openNative(path) {
	if (process.platform === "win32") {
		// One verbatim argument: node's default re-quoting escapes quotes with
		// backslashes, which cmd.exe does NOT understand (it silently fails).
		// Passing the whole `/c start "" "path"` line verbatim lets cmd parse
		// the cmd-style quotes itself.
		try {
			const line = `/c start "" "${path.replace(/"/g, "")}"`;
			const child = spawn(CMD_EXE, [line], {
				detached: true,
				stdio: "ignore",
				windowsHide: true,
...
function revealNative(path) {
	if (process.platform === "win32") {
		// windowsVerbatimArguments passes the argument EXACTLY as written, so
		// explorer receives `/select,"C:\path with spaces\file"` and can parse
		// the quoted path itself (node's default re-quoting breaks it).
		try {
			const child = spawn(EXPLORER_EXE, [explorerSelectArg(path)], {
				detached: true,
				stdio: "ignore",
				windowsHide: true,
				windowsVerbatimArguments: true
