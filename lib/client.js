...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * A file counts as produced when its tool result carries a render
		 * intent that creates/modifies it (diff card, or a generic edit) —
		 * the same rule ui-deliverables uses. Results are deduped by path
		 * (last occurrence wins), ordered by seq, capped at RECENT_LIMIT,
		 * most-recent-first.
		...
		function producedFilesFromSnapshot(nodes) {
			...
			for (const node of nodes) {
				if (node === null || typeof node !== "object" || node.kind !== "tool-result") continue;
				...
				for (const loc of locations) {
					if (loc !== null && typeof loc === "object" && typeof loc.path === "string" && loc.path.length > 0) {
						entries.push({ path: loc.path, seq: node.seq ?? 0, time: node.time ?? 0 });
					}
				}
			}
		...
		function RecentRow({ item, t, openFile }) {
			...
			const Icon = kind === "image" ? IconDataOutline16 : kind === "pdf" ? IconDataOutline16 : IconCodeOutline16;
